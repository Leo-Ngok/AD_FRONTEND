...
function refreshBreadCrumb() {
  routeMatched.value = route.matched.filter(
    (item) => item.meta.breadcrumb !== false && !item.meta.hidden
  )
  ...
  if (routeMatched.value[0].path !== '/dashboard') {
    routeMatched.value.unshift(<RouteLocationMatched>{
      ...
      path: resolve('/', dashboardRoute.children?.[0].path as string),
    })
  }
}
</script>
